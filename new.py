...
class GeometricFlow(nn.Module):
    """ Replaces FFN with a manifold-constrained flow (Inflation/Deflation) """
    def __init__(self, d):
        super().__init__()
...
class ManifoldAttention(nn.Module):
    """ Attention as a distance-based similarity on a hypersphere """
    def __init__(self, d):
        super().__init__()
...
class StandardAttention(nn.Module):
    ...
    def forward(self, x, mask):
        x_norm = self.norm(x)
        # Convert causal mask to bool attn_mask
        attn_mask = (1 - mask).bool()
        out, _ = self.mha(x_norm, x_norm, x_norm, attn_mask=attn_mask, need_weights=False)
        return x + out
...
class UnifiedTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Helical Position Encoding (Geometric Logic)
...
def generate(model, prompt, length=50):
    model.eval()
    idx = torch.tensor([stoi[c] for c in prompt if c in stoi], device=device).unsqueeze(0)
    if idx.shape[1] == 0: idx = torch.zeros((1,1), dtype=torch.long, device=device)
